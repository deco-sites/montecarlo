...
export interface VariantsShelf {
  index: number;
  groupVariants: GroupVariants[] | null;
}
...
const loader = async (props: PropsLoader): Promise<VariantsShelf[] | null> => {

  if (props.product === null || props.product === undefined) {
    return null;
  }

  async function getGroupCode(product: Product) {

    const { isVariantOf, additionalProperty, inProductGroupWithID, productID } = product;
    ...
    if (typesToBeRendered && result && typesToBeRendered.length > 0) {
      ...
      typesToBeRendered.map((type) => {
        ...
        if (type == "Cor do metal" && materialProperty) {
          ...
          arrayVariant.push({ type: "Material", variants: variants, productID: productID });
        }
        if (type == "pedra" && stoneProperty) {
          ...
          arrayVariant.push({ type: "Pedras", variants: variants, productID: productID });
        }
        if (type == "Tamanho" && (sizeCorrenteProperty || sizeProperty)) {
          ...
          arrayVariant.push({ type: "Tamanho", variants: variants, productID: productID });
        }
        if (type == "Letra" && LetterProperty) {
          ...
          arrayVariant.push({ type: "Letras", variants: variants, productID: productID });
        }
        if (type == "cts" && ctsProperty) {
          ...
          arrayVariant.push({ type: "Cts Diamantes", variants: variants, productID: productID });
        }
        if (type == "variacoes" && result.length > 1) {
          ...
          arrayVariant.push({ type: "Variações", variants: variants, productID: productID });
        }
      });
      return arrayVariant;
    }
  ...
  const variants: VariantsShelf[] | null = []
