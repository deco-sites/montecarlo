        ...
        <div class="flex flex-col gap-5 container py-10 px-5 md:grid md:grid-cols-2 md:px-14 md:py-14 max-w-[1440px]">
            {
                title || description ? (
                    <div class="flex flex-col gap-3 px-10">
                        { title ? <h2 class="font-poppins text-2xl md:text-[40px] md:leading-[44px]">{title}</h2> : null }
                        { description ? <p class="font-poppins text-sm font-medium">{description}</p> : null }
                    </div>
                ) : null
            }
                                            ...
                                            <City key={index} title={city.label}>
                                                {
                                                    city.stores?.map((store, index) =>
                                                        ...
                                                        <div key={index} class="flex flex-col gap-5 mx-5">
                                                            <h5 class="font-poppins font-semibold text-sm">{store.label}</h5>
                                                            {
                                                                store.address ? (
                                                                    <div class="flex flex-col gap-2">
                                                                        <span class="font-poppins text-xs uppercase">Endereço:</span>
                                                                        <div class="font-poppins text-sm" dangerouslySetInnerHTML={{ __html: store.address }}/>
                                                                    </div>
                                                                ) : null
                                                            }
                                                            {
                                                                store.phone && store.phone.length > 0 ? (
                                                                    <div class="flex flex-col gap-2">
                                                                        <span class="font-poppins text-xs uppercase">Telefone:</span>
                                                                        {
                                                                            store.phone.map((phone, index) => 
                                                                                <a class="font-poppins text-sm" key={index} href={phone.href}>{phone.label}</a>
                                                                            )
                                                                        }
                                                                    </div>
                                                                ) : null
            ...
            {
                sac ? <div class="font-poppins text-sm px-10 md:py-10" dangerouslySetInnerHTML={{ __html: sac }} /> : null
            }
        </div>
    )
}
...
function State (props: InfoProps) {
    ...
    return (
        <details class={`group/State`}>
            <summary class={`flex cursor-pointer items-center justify-between bg-[#F5F3E7] py-2 px-3`}>
                <h4 class={`group-open/State:font-semibold font-poppins text-sm ml-2`}>{title}</h4>
                <Icon
...
function City (props: InfoProps) {
    ...
    return (
        <details class={`group/City`}>
            <summary class={`flex cursor-pointer items-center justify-between bg-[#F8F7F3] py-2 px-3`}>
                <h4 class={`group-open/City:font-semibold font-poppins text-sm ml-2`}>{title}</h4>
                <Icon
