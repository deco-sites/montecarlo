...
function Flags(
  { productAdditionalProperty, releaseFlag, discountFlag, isProductPage }:
    Props,
) {
  const releaseProduct = productAdditionalProperty?.filter(
    (eachAdditionalProperty) => {
      eachAdditionalProperty.value == "Lançamentos";
    },
  );
  let isReleaseProduct = releaseProduct && releaseProduct.length > 0;

  const customFlagProduct = productAdditionalProperty?.filter(
    (eachAdditionalProperty) => {
      eachAdditionalProperty.name == "flagCustomizada";
    },
  );
  ...
  if (customFlagProduct && isCustomFlagProduct) {
    const properties = customFlagProduct[0].value
      ? customFlagProduct[0].value
      : "";
  ...
  return (
    <>
      ...
      <div
        className={`${
          isProductPage ? "relative top-0 left-0" : "absolute z-10 top-5 left-3"
        }`}
      >
        ...
        {isReleaseProduct
          ? (
            <ReleaseFlag
              ...
              text={releaseFlag?.text}
              backgroundColor={releaseFlag?.backgroundColor}
              fontColor={releaseFlag?.fontColor}
            />
          )
          : ""}
        {propertiesFormatted
          ? (
            <CustomFlag
              isProductPage={isProductPage}
              text={propertiesFormatted.textoFlag}
              textColor={propertiesFormatted.corTexto}
              backgroundColor={propertiesFormatted.corFundo}
            />
          )
          : ""}
      </div>
      <div className={`${isProductPage ? "hidden" : "block"}`}>
        <DiscountFlag
          isProductPage={isProductPage}
          oldPrice={discountFlag?.oldPrice}
          newPrice={discountFlag?.newPrice}
          initialText={discountFlag?.initialText}
          finalText={discountFlag?.finalText}
          backgroundColor={discountFlag?.backgroundColor}
          fontColor={discountFlag?.fontColor}
        />
      </div>
    </>
  );
