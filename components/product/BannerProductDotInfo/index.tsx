...
function BannerProductDotInfo(props: Props) {
    ...
    function handleDot(event: MouseEvent) {
        ...
        dot.setAttribute("data-active", dot.getAttribute("data-active") === "false" ? "true" : "false");
    }
    
    function DotInfo({id, coords}:{id: number, coords: { mobile: string; desktop: string }})  {
        const posMobile = coords.mobile.split(":");
        const posDesktop = coords.desktop.split(":");
    
        return (
            <div class={`${dotsActive ? "visible opacity-100" : "invisible opacity-0"} transition-opacity`}>
                <div
