...
export default function Info({ alignment, coordinates, id }: Props) {
    ...
    useEffect(() => {
        async function fetchProductData() {
            try {
                const response = await fetch(`https://montecarlo.vtexcommercestable.com.br/api/catalog_system/pub/products/offers/${productID}`);
                ...
                setProductData({
                    ...
                    oldPrice: formatNumber.format(data[0].Offers[0].OffersPerSalesChannel[0].PriceWithoutDiscount),
                    price: formatNumber.format(data[0].Offers[0].OffersPerSalesChannel[0].ListPrice),
                });
