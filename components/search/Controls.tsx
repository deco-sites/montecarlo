        ...
        <>
          <div class="bg-base-100 flex flex-col h-full divide-y overflow-y-hidden">
            <div class="flex justify-between items-center w-[80vw] max-w-[500px] pr-2">
              ...
              <Button class="btn btn-ghost bg-transparent hover:bg-transparent border-none ring-transparent" onClick={() => open.value = false}>
    ...
    >
      {
        layout === "horizontal" ? (
          <>
            <div class="hidden md:flex flex-col gap-2">
              ...
              <div class={`flex ${layout === "horizontal" ? "flex-row gap-4" : "flex-col gap-6"} p-4 md:pl-0`}>
                 <ul ref={filtersRef} className={`flex flex-row gap-4 flex-wrap ${!moreFilters.value ? "overflow-hidden max-h-[20px]" : ""}`}>
                  ...
                  {showButton.value === true ? (
                    <button
                      ...
                      onClick={() => { moreFilters.value = !moreFilters.value; }}
                    >
            ...
            <div class={`grid grid-cols-2 md:flex md:flex-row flex-wrap items-center justify-between sm:gap-4 w-full ${!displayFilter ? "" : ""} md:hidden`}>
              ...
              <div class={"flex col-span-2 justify-end w-full gap-2 my-2"}>
                <button
                  class={`p-1 border ${columnsMobile.value === 1 ? "border-[##AAA89C]" : "border-transparent"}`}
                  onClick={() => columnsMobile.value = 1}>
                  <Icon id="columnOne" width={32} height={16} />
                </button>
                <button
                  class={`p-1 border ${columnsMobile.value === 2 ? "border-[##AAA89C]" : "border-transparent"}`}
              ...
              <div class="flex flex-row gap-2 col-span-2">
                <Button
                  class={`bg-[#F5F3E7] ${displayFilter ? "btn-ghost" : "btn-ghost md:hidden"} flex-1`}
                  onClick={() => {
                    open.value = true;
                  }}
                >
          ...
          <div class="flex flex-col justify-between sm:p-0 sm:gap-2 sm:flex-col">
            <div class={`grid grid-cols-2 md:flex md:flex-row flex-wrap items-center justify-between sm:gap-4 w-full ${!displayFilter ? "" : ""}`}>
              <div class="flex flex-col gap-2 sm:p-0 mb-2 col-span-2">
                {
                  title && title.length > 0 ? (
                    <h6 class={"font-poppins text-sm font-semibold"}>{title}</h6>
